...
class PasanteController extends Controller
{
    ...
    public function verBitacora(Request $request)
    {
        ...
        $query = BitacoraEva::with(['actividad', 'jefe.user'])
            ->where('idU_pasante', $pasante->idU_pasante);
        
        if ($request->has('id_pasantia')) {
            $query->whereHas('actividad', function($q) use ($request) {
                $q->where('id_pasantia', $request->id_pasantia);
            });
        }
    ...
    // Calificar pasantía (al finalizar)
    ...
    public function misCalificaciones()
    {
        ...
        $calificaciones = Comentario::with('pasantia.empresa')
            ->where('idU_pasante', $pasante->idU_pasante)
            ->get();
