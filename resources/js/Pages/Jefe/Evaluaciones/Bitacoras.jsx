...
export default function Bitacoras({ pasantia, pasantesData = [], auth }) {
    ...
    const verActividadGlobal = (id_actividad) => {
        router.get(`/jefe/actividades/${pasantia.id}`);
    };
    ...
    return (
        <DashboardLayout auth={auth}>
            ...
            <div className="mb-8 p-6 bg-white border border-slate-100 rounded-2xl shadow-sm">
                <span className="text-xs font-bold tracking-wider text-primary-blue uppercase bg-primary-blue/10 px-3 py-1 rounded-full">
                    Programa de Pasantía
                </span>
                <h1 className="text-3xl font-black text-primary-navy uppercase mt-2">
                    {pasantia.nombre}
                </h1>
            ...
            {pasantesData.length === 0 ? (
                <div className="bg-white border text-center p-12 rounded-2xl text-slate-400">
                    No existen pasantes inscritos o asignados a su cargo bajo
                    esta pasantía.
                </div>
            ) : (
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-full bg-slate-100 rounded-full h-2.5">
                                                                <div
                                                                    ...
                                                                    style={{
                                                                        width: `${actividad.porcentaje_progreso}%`,
                                                                    }}
                                                                ></div>
                                                            </div>
                                                    ...
                                                    <td className="py-4 px-6 text-right">
                                                        <div className="flex items-center justify-end gap-2.5">
                                                            ...
                                                            <button
                                                                ...
                                                                className="flex items-center justify-center w-10 h-9 bg-slate-100 text-slate-600 hover:bg-slate-800 hover:text-white rounded-xl transition-all transform hover:-translate-y-0.5 shadow-sm"
                                                            ...
                                                            <button
                                                                ...
                                                                className={`flex items-center justify-center w-9 h-9 rounded-xl transition-all shadow-sm ${
                                                                    actividad.tiene_autoevaluacion
                                                                        ? "bg-purple-100 text-purple-700 hover:bg-purple-600 hover:text-white transform hover:-translate-y-0.5"
                                                                        : "bg-gray-100 text-gray-400 cursor-not-allowed opacity-60"
                                                                }`}
                                                                title={
                                                                    actividad.tiene_autoevaluacion
                                                                        ? "Ver Autoevaluación del alumno"
                                                                        : "Sin autoevaluación"
                                                                }
                                                            >
                                                                <UserCheck
                                                            ...
                                                            <button
                                                                onClick={() =>
                                                                    abrirEvaluacion(pasante, actividad)
                                                                }
                                                                // El botón se deshabilita si "puede_evaluar" es falso
                                                                disabled={
                                                                    !actividad.puede_evaluar
                                                                }
                                                                className={`flex items-center justify-center w-9 h-9 rounded-xl transition-all shadow-sm ${
                                                                    !actividad.puede_evaluar
                                                                        ? "bg-gray-100 text-gray-400 cursor-not-allowed shadow-none" // Deshabilitado
                                                                        : actividad.tiene_bitacora
                                                                          ? "bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transform hover:-translate-y-0.5" // Habilitado con bitácora
                                                                          : "bg-amber-500 text-white hover:bg-amber-600 shadow-amber-200 transform hover:-translate-y-0.5" // Habilitado sin bitácora
                                                                }`}
                                                                title={
                                                                    !actividad.puede_evaluar
                                                                        ? "Actividad no disponible para evaluación"
                                                                        : actividad.tiene_bitacora
                                                                          ? "Ver Bitácora Evaluada"
                                                                          : "Evaluar Actividad"
                                                                }
                                                            >
                                                                <ClipboardCheck
            ...
            <Modal
                show={progresoModal}
                onClose={() => setProgresoModal(false)}
                title={`Historial de Progresos: ${selectedActividadNombre}`}
                maxWidth="2xl"
            >
                <div className="p-1">
                    {selectedProgresoList.length === 0 ? (
                        <p className="text-slate-400 text-sm text-center py-6">
                            El estudiante no ha registrado avances en esta
                            actividad todavía.
                        </p>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead>
                                    <tr className="border-b text-xs font-bold uppercase text-slate-400 bg-slate-50">
                                        <th className="p-3 w-3/12">
                                            Fecha / Hora
                                        </th>
                                        <th className="p-3 w-2/12 text-center">
                                            Avance
                                        </th>
                    ...
                    <div className="flex justify-end mt-4">
                        <SecondaryButton
                            onClick={() => setProgresoModal(false)}
                        >
                            Cerrar ventana
                        </SecondaryButton>
                    </div>
            ...
            <Modal
                show={autoEvaModal}
                onClose={() => setAutoEvaModal(false)}
                title={`Autoevaluación del Pasante: ${selectedActividadNombre}`}
                maxWidth="xl"
            >
                <div className="space-y-4">
                    {selectedAutoEva ? (
                        <>
                            ...
                            <div>
                                <InputLabel
                                    value="Comentario reflexivo del estudiante:"
                                ...
                                <div className="mt-1.5 p-4 bg-slate-50 border border-slate-100 rounded-xl text-slate-600 text-sm leading-relaxed whitespace-pre-line">
                                    {selectedAutoEva.comentario ||
                                        "El estudiante no adjuntó comentarios a su autoevaluación."}
                                </div>
                            </div>
                        </>
                    ) : (
                        <p className="text-slate-400 text-center py-4">
                            No hay datos válidos registrados.
                        </p>
                    )}

                    <div className="flex justify-end pt-2 border-t border-slate-100">
                        <SecondaryButton onClick={() => setAutoEvaModal(false)}>
                            Regresar a la lista
                        </SecondaryButton>
                    </div>
                </div>
            ...
            <Modal
                ...
                title={
                    modalMode === "create"
                        ? "Evaluar y Registrar Bitácora"
                        : "Detalle de Bitácora Auditada"
                }
                maxWidth="2xl"
            >
                <form onSubmit={handleEvaluarSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl">
                        <InfoItem
                            icon={"User"}
                            label="Pasante"
                            value={activePasante?.nombre_completo || ""}
                        />
                        <InfoItem
                            icon={"ClipboardListIcon"}
                            label="Actividad a calificar"
                            value={activeActividad?.nombre_act || ""}
                        />
                    </div>
                    ...
                    <div>
                        <InputLabel
                            htmlFor="nota"
                            value="Calificación Asignada (0 - 100)"
                        />
                        <TextInput
                            id="nota"
                            type="number"
                    ...
                    <div>
                        <InputLabel
                            htmlFor="observacion"
                            value="Observaciones / Retroalimentación del Jefe"
                        />
                        <TextArea
                            id="observacion"
